...
We now prove the correctness of the presented Byzantine Generalized Paxos protocol. Invariants and symbols specific to our proof are defined in Table~\ref{table:1}. Due to space constraints, we only discuss the proof of consistency, but the remaining proofs and an extended version of the protocol to address cross-ballot consistency are available in a technical report~\cite{bgp:tr}.
\vspace{-0.5cm}
\begin{table}[h!]
