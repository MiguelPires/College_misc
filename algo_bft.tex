...
\begin{algorithm} 
	...
	\begin{algorithmic}[1]
		...
		\State \hspace{\algorithmicindent}\hspace{\algorithmicindent} \textbf{if} $!\Call{Contains}{c, seq}$ \textbf{then}
		\State \hspace{\algorithmicindent}\hspace{\algorithmicindent}\hspace{\algorithmicindent} $acc\_seq =  acc\_seq \bullet c$;
		\State \hspace{\algorithmicindent} \textbf{end for}
		\State \textbf{end for}
		\State \textbf{return} $acc\_seq$;
		\EndFunction
...
\begin{algorithm} 
	...
	\begin{algorithmic}[1]
		...
		\State $k = max(i\ |\ (i < m) \wedge (\exists a \in Q :\ val_a[i]\ \neq null))$;
		\If {$bal_a == k$ and $!\Call{conflict}{value, val_a}$}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State \hspace{\algorithmicindent}\hspace{\algorithmicindent}\hspace{\algorithmicindent}\hspace{\algorithmicindent} \textbf{if} $!\Call{Contains}{c, learned}$ \textbf{then}
