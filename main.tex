...
\begin{algorithmic}[1]
    ...
    \Function{Propose}{\textit{C}}
        ...
        \Else
            \State \textbf{run} \Call{send}{\textit{propose, C}} to Acceptors;
        \EndIf
    \EndFunction    
...
%\Comment{$bal_c is the last ballot c has started and maxTried_c is the c-struct it suggested at bal_c$}
\begin{algorithmic}[1]
    ...
    \State \textbf{upon} $receive (p1b, ballot_l, bal_i, val_i)$ from Q
        \State \hspace{\algorithmicindent} \textbf{run} \Call{phase\_2a}{$ballot_l, Q$};
    ...
    \Function{Phase\_2a}{$bal, Q$}
        \State $maxTried_l$ = \textbf{run} \Call{ProvedSafe}{$Q, bal$};
    ...
    \Function{Proved\_Safe}{Q, m}
        ...
        \State $R = \{R \in k$-$quorum\ |\ \forall_a \in R \cap Q : val_a[k] \neq null\}$;
...
\begin{algorithmic}[1]
  ...
  \State \textbf{upon} \textit{receive(propose, val)} from proposer \textit{p} \textbf{do}
    ...
    \State
    \State \textbf{upon} \textit{receive(p1a, ballot)} from acceptor \textit{a} \textbf{do}
    \State \hspace{\algorithmicindent} \textbf{run} \Call{phase\_1b}{$ballot$};
    
    \State
    \State \textbf{upon} \textit{receive(p2a, ballot, value)} from acceptor \textit{a} \textbf{do}
    \State \hspace{\algorithmicindent} \textbf{run} \Call{phase\_2b\_classic}{$ballot, value$};
    
    \State
    \Function{Phase\_1b}{$m$}
        \If {$bal_a < m$}
            \State $bal_a = m$;
            \State \Call{send}{$p1b, m, mbal_a, val_a$} to coord($m$);
        \EndIf
    \EndFunction
    
    \State
    \Function{Phase\_2b\_Classic}{$m, u$}
        \If {$bal_a \leq m$}
            \State $bal_a = m$;
            \State $val_a = u$;
