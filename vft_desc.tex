...
Next, we describe solely the agreement subprotocol since it's the only one affected by the Visigoth model\todo{Not true}. The view change subprotocol is identical to the one used in \acrlong{bgp}.
...
\noindent\textbf{Step 2: Acceptors to acceptors.} In order for the acceptors to be able to prove that, with respect with some sequence that they voted for, no other non-commutative sequence may have been learned, they broadcast their signature of a tuple with the current ballot and the sequence. After gathering $N-f$ such proofs and validating their signatures, the acceptors can prove to the leader that at least $f+1$ correct acceptors voted for the same sequence which means that no non-commutative sequence may have been learned since there are only another $2f$ acceptors in the system\looseness=-1.\par
