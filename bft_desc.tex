...
\begin{algorithm} 
	...
	\begin{algorithmic}[1]
		...
		\State \hspace{\algorithmicindent} \parbox{\linewidth}{\textbf{if} $(\#(proofs[ballot_i][val_i]) \geq N-f$ or ($\#(proofs[ballot_i][val_i]) > f$ and \Call{isUniversallyCommutative}{$val_i$})) and $proofs[ballot_i][val_i][a] \neq \bot$ \textbf{then}}
		...
		\If {$ballot \geq bal_a$ and $proven == \Call{subsequence}{value, 0, \#(proven)}$}
		\State $bal_a = ballot$;
		\State $val_a = value$;
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]			
		...
		\parbox{\linewidth}{\textbf{if} $\#(messages[ballot][value]) \geq N-f$ or ($\#(messages[ballot][value]) > 1$ and \Call{isUniversallyCommutative}{value}) \textbf{then}}
...
The correctness of the protocol is heavily dependent on the guarantee that each vote sent by an acceptor is a prefix of each subsequent vote. Otherwise, if the network rearranges phase $2b$ messages such that they're seen by the different learners in different orders, they will result in a different final state. If, however, every vote is a prefix of all subsequent votes then, regardless of the order in which the sequences are learned, the final state will be the same. \par 
This is ensured by the correct execution of the protocol since every vote in a fast ballot is equal to the previous vote with a sequence appended at the end and every vote in a classic ballot is equal to all the learned votes concatenated with unlearned votes and new proposals. However, this begs the question of how the protocol fares when Byzantine faults occur. In particular, the worst case scenario occurs when both $f$ acceptors and the leader are Byzantine (remember that a process can have multiple roles, such as leader and acceptor). In this scenario, the leader can purposely sends phase $2a$ messages for a sequence that is not prefixed by the previously accepted values. Coupled with an asynchronous network, this malicious message can be delivered before the correct votes of the previous ballot, resulting in different acceptors learning different sequences that may not be extensible to equivalent sequences. \par
To prevent this scenario, the acceptors must ensure that the proposals they receive from the leader are prefixed by the values they have previously voted for. Since an acceptor votes for its $val_a$ sequence after receiving $N-f$ verification votes for an equivalent sequence and stores it in its $proven$ variable, the acceptor can verify that the it's $proven$ sequence is an eq-prefix of the leader's proposed value (i.e., $proven \sqsubseteq value$). A practical implementation of this condition is simply to verify that the $value$'s subsequence starting at the index $0$ up to index $length(proven)-1$ is equivalent to the acceptor's $proven$ sequence. 
