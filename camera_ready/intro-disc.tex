...
Section~\ref{sec:protocol} presents the Generalized Paxos protocol that is resilient against Byzantine failures. Section~\ref{proof} presents a proof that the Byzantine Generalized Paxos protocol guarantees \textit{consistency}, while the correctness proofs for the remaining properties are included in a tech report. Section~\ref{sec:disc} concludes the paper with a discussion of several optimizations and practical considerations. The complete tech report with the formal proofs
